...
public:
    ...
    */
    double evaluate(const std::vector<FeatureType>& X, 
                    ...
                    const FeatureType& bias) const override {
    
        std::size_t num_samples = y.size();
        std::size_t num_features = weight.size();  
        double loss = 0.0;
        double X_w, y_hat;
        ...
        double reg = std::inner_product(weight.begin(), 
                                        weight.end(), 
                                        weight.begin(), 0.0);
        reg /= static_cast<double>(num_samples);
        std::cout << loss_param_.at("alpha") << std::endl;
    ...
    */
    void gradient(const std::vector<FeatureType>& X, 
                  ...
                  FeatureType& bias_grad) const override {
        ...
        for (std::size_t i = 0; i < num_samples; ++i) {
            ...
            bias_grad += (y_hat[i] - static_cast<FeatureType>(y[i]));
        }
        bias_grad /= static_cast<FeatureType>(num_samples);
        ...
        while (feature_index < num_features) {
            ...
            weight_grad[feature_index] = inner_prod / static_cast<FeatureType>(num_samples) + 
                                         loss_param_.at("alpha") * 2.0 * weight[feature_index];
            ++feature_index;
            inner_prod = 0.0;
        }
    }
