...
public:
    ...
    void optimize(const std::vector<sgdlib::FeatureScalarType>& X,
                  const std::vector<sgdlib::LabelScalarType>& y) override {
        ...
        // initialize gradient,
        sgdlib::FeatureScalarType y_hat, grad, loss;
    ...
    void optimize(const sgdlib::ArrayDatasetType& dataset) override {
        ...
        for (iter = 0; iter < this->max_iters_; ++iter) {
            ...
            // set wscale
            this->loss_fn_->set_param("wscale", wscale);
