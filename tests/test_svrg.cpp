...
#include "sgdlib/algorithm/gd/svrg.hpp"

class SVRGTest : public ::testing::Test {
public:
    virtual void SetUp() {
        X_train = {
...
TEST_F(SVRGTest, SVRGOptimizerTest) {
    ...
    // check weight update
    EXPECT_EQ(coef.size(), 4);
    EXPECT_FALSE(coef[0] == w0[0]) << "Weights not updated";
    EXPECT_FALSE(coef[1] == w0[1]) << "Weights not updated";
