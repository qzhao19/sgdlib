...
// ****************************vecadd 3*******************************//
TYPED_TEST(MathOpsSSETest, VecAdd1XWithConstCSSETest) {
    using T = typename TestFixture::Type;
    ...
    // small size n = m = 5 > 4
    std::vector<T> out5(5), out5_ref(5);
    sgdlib::detail::vecadd_sse<T>(x8.data(), c, 5, out5.data());
    sgdlib::detail::vecadd_ansi<T>(x8, c, out5_ref);
    for (std::size_t i = 0; i < 5; ++i) {
        EXPECT_FLOAT_EQ(out5[i], out5_ref[i]);
        // std::cout << out5_ref[i] << " ";
    }
    std::cout << std::endl;
    ...
    sgdlib::detail::vecadd_sse<T>(x_large.data(), c, 1025, out1025_1.data());
    ...
    sgdlib::detail::vecadd_sse<T>(this->aligned_mem_vec, c, size6, aligned_mem_vec_out.data());
    ...
    sgdlib::detail::vecadd_sse<T>(this->unaligned_mem_vec, c, size6, unaligned_mem_vec_out.data());
    ...
    for (auto chunk_size : chunk_sizes) {
        ...
        while (processed < size71) {
            ...
            sgdlib::detail::vecadd_sse<T>(x71.data() + processed, c, current_chunk, out2.data());
            // EXPECT_NEAR(expected7, actual7, 1e-3);
            for (size_t i = 0; i < current_chunk; ++i) {
                EXPECT_FLOAT_EQ(out1[i], out2[i]);
            }
    ...
    auto start = std::chrono::high_resolution_clock::now();
    sgdlib::detail::vecadd_sse<T>(x_huge.data(), c, huge_size, out_huge1.data());
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> elapsed1 = end - start;
    std::cout << "vecadd 1 input with constant C SIMD execution time: " << elapsed1.count() << " seconds\n";

    start = std::chrono::high_resolution_clock::now();
    sgdlib::detail::vecadd_ansi<T>(x_huge, c, out_huge2);
    end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> elapsed2 = end - start;
    std::cout << "vecadd 1 input with constant C ANSI execution time: " << elapsed2.count() << " seconds\n";
    std::cout << "Speedup: " << elapsed2.count() / elapsed1.count() << "x\n";
...
// ****************************vecaccumul******************************//
TYPED_TEST(MathOpsSSETest, VecAccumulSSETest) {
    using T = typename TestFixture::Type;
    ...
    EXPECT_FLOAT_EQ(expected31, sgdlib::detail::vecaccmul_sse<T>(x3.data(), x3.data() + x3.size(), x3.size()));

    // elements with Nan
    std::vector<T> x4 = {1.0, 2.0, std::numeric_limits<T>::quiet_NaN(), 4.0};
    T expected41 = sgdlib::detail::vecaccmul_ansi(x4.data(), x4.data() + x4.size());
    EXPECT_TRUE(std::isnan(expected41));
    EXPECT_TRUE(std::isnan(sgdlib::detail::vecaccmul_sse<T>(x4.data(), x4.data() + x4.size(), x4.size())));
    ...
    EXPECT_FLOAT_EQ(expected51, sgdlib::detail::vecaccmul_sse<T>(x5.data(), x5.data() + x5.size(), x5.size()));
    ...
    EXPECT_FLOAT_EQ(expected61, sgdlib::detail::vecaccmul_sse<T>(this->aligned_mem_vec, this->aligned_mem_vec + size6, size6));
    ...
    for (auto chunk_size : chunk_sizes) {
        ...
        while (processed < size7) {
            ...
            T actual7  = sgdlib::detail::vecaccmul_sse<T>(x7.data() + processed, x7.data() + processed + current_chunk, current_chunk);
            EXPECT_NEAR(expected7, actual7, 1e-3);
            processed += current_chunk;
        }
    }
    ...
    auto start = std::chrono::high_resolution_clock::now();
    T out1 = sgdlib::detail::vecaccmul_sse<T>(x_huge.data(), x_huge.data() + huge_size, huge_size);
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> elapsed1 = end - start;
    std::cout << "vecnorm2 SIMD execution time: " << elapsed1.count() << " seconds\n";
